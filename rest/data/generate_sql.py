...
def generate_sql():
    with open("fixed_stats.json", "r", encoding="utf-8") as f:
        fixed_stats = json.load(f)

    with open("rilevazioni.json", "r", encoding="utf-8") as f:
    ...
    for stazione in fixed_ril["stazioni"]:
        for misurazioni in stazione["misurazioni"]:
            for inq, mis_inq in misurazioni.items():
                for mis in mis_inq:
                    sql_rilevazioni += f"\n{stazione['codseqst'], mis['data'], inq, float(mis['mis']) if mis['mis'] != '' else 0.0},"
...
def fix_data():
    ...
    with open("fixed_stats.json", "w", encoding="utf-8") as f:
